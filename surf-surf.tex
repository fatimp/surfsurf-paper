...
\author{Nikolay{\thinspace}M.~Evstigneev\textsuperscript{4}}
...
\affiliation{\textsuperscript{4}Federal Research Center ``Computer Science and
  Control'' of the Russian Academy of Sciences, Moscow, Russia.}
...
\begin{abstract}
  ...
  importance and recently Ma \& Torquato [Phys. Rev. E 98, 013307 (2018)]
  ...
  Poisson disks in case the resolution of the image is adequate. Based on the
  multi-scale image analysis we developed a $C_{0.5}$ criterion that can predict
  if the imaging resolution is enough to make an accurate evaluation of the
  surface CFs. We also showed that in case when the input image contains all
  major features, but do not pass the $C_{0.5}$ criterion, it is possible with
  the help of image magnification to sample CFs almost similar to those obtained
  for high-resolution image of the same structure with high $C_{0.5}$. The
  computational framework as developed here is open source and available within
  the \code{CorrelationFunctions.jl} package developed by our group. Our
  "digital" approach was applied to a wide variety of real porous media images
  of different quality. We discuss critical aspects of surface correlation
...
\cite{Gommes1,Gommes2,Degeneraty.045306} and, thus, allows reconstructing and
performing stationarity/representativeness characterization \cite{REVpaper} for
...
algorithm of Ma and Torquato\cite{ma2018SS}. It has better complexity than the
algorithm of Torquato. There are two steps in our algorithm: edge detection and
computation of autocorrelation. The first step has $O(n)$ complexity and the
second step has $O(n \log n)$ complexity where $n$ is a number of pixels (or
voxels) in the input. Ma's and Torquato's algorithm has $O(n)$ complexity for
finding intersections with probe rays and $O(m^2)$ for computation of
correlation functions where $m$ is a number of found intersections. Therefore,
performance of Ma's and Torquato's algorithm depends on the input (more
specifically, on the number of intersections of the interface and probe rays)
and may be unsatisfying when $n \sim m$.

Ma's and Torquato's algorithm is very noisy (at least our implementation and an
implementation provided with the paper are). The noise does not disappear even
with increase in resolution of the input image (\cref{fig:torquato}).

In the case of overlapping disks, Torquato's algorithm produces results which
are farther from the theory than ours (\cref{fig:torquato}).

Their algorithm is very sensitive to selection of standard deviation $b$ of
Gauss filter and it is not clear how to select this parameter properly
(\cref{fig:torquato-param}). Finally, Ma and Torquato do not provide any
criterion of applicability for their algorithm.

Ma and Torquato also claim that their algorithm can be applied to grayscale
images directly. This statement requires further clarification because it is not
clear how an indicator function is defined in this case. In authors'
implementation there is a parameter $T$ and the function is defined as
\begin{equation*}
  I^{(solid)}(\mathbf{x}) = \Theta(A(\mathbf{x}) - T)
\end{equation*}
where $A(\mathbf{x})$ is an intensity of a pixel in the image at the coordinate
$\mathbf{x}$. This approach gives a result which is highly sensitive to a choice
of $T$ (\cref{fig:torquato-segm}).
...
\begin{figure*}[!pt]
  \centering
  \subfigure[Image dimensions $1500 \times 1500$, $C_{0.5} = 0.880$.]{
    \includegraphics[width=0.45\linewidth]{images/torquato-sigma-small.png}
    \label{fig:torquato-sigma-small}}
  \hfill
  \subfigure[Image dimensions $5000 \times 5000$, $C_{0.5} = 0.934$.]{
    \includegraphics[width=0.45\linewidth]{images/torquato-sigma.png}
    \label{fig:torquato-sigma}}
  \caption[]{Illustration of how a choice of $b$ affects Ma's and
    Torquato's algorithm. Two cases of images of overlapping disks are
    shown. Our algorithm cannot process \cref{fig:torquato-sigma-small}, but
    works good for \cref{fig:torquato-sigma}, while Ma's and Torquato's
    algorithm produce results highly dependant on $b$ in the both cases.}
...
\begin{figure*}[!pt]
  ...
  \subfigure[Grayscale image.]{
    \includegraphics[width=0.45\linewidth]{images/241-2-1.png}
    \label{fig:torquato-sandstone}}
  \hfill
  \subfigure[Surface-surface correlation function.]{
    \includegraphics[width=0.45\linewidth]{images/torquato-sandstone.png}
    \label{fig:torquato-sandstone-ss}}
  \caption[]{A two-dimensional slice of coarse-grained sandstone
    (\cref{fig:torquato-sandstone}) and surface-surface correlation functions
    computed with different threshold parameter $T$
    \cref{fig:torquato-sandstone-ss}.}
  \label{fig:torquato-segm}
\end{figure*}
