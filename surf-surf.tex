...
\begin{figure*}[t]
  ...
  \subfigure[Image size $64 \times 64$, disk radius 0.96 pixels, $C_{0.5} = 0.6549$]{
    ...
    \label{fig:disks-64}}
  \caption[]{The 2D images of overlapping disks (a-d) and value noise (e-h) with
    different resolution (\cref{sec:results}). \textcolor{red}{Vasya, please,
      put disks to the upper row, and move value noise below, also, please, add
      a frame around each image for visibility, e.g., like it is on Fig.15 you
      have below}}
...
perfect for discretization of about 61 pixels for each disk's diameter. A
...
the image: linear and bicubic interpolation, and nearest-neighbor interpolation.
...
\begin{figure*}[ht]
  \centering \subfigure[The image resulting from digitizing of Poisson disks to
  ...
  \hfill \subfigure[Digitized image of Poisson disks with the size of
  ...
  \caption[]{An example of images with almost identical correlation functions
    obtained using different methods: direct discretization from known disk
    positions and magnification using bilinear
    interpolation. \textcolor{red}{Vaslya, please: combine this figure and the
      next one into a single one, make frames for the images and also scale them
      a bit so that would not cover so much space, for CFs legends please rename
      resize->magnified, calculated->discretized}}
  \label{fig:resized}
\end{figure*}
...
\begin{figure}[ht]
  ...
  \caption[]{Decay of criterion $C_{0.5}$ as a function of the image resolution
    for the downscaled image (shown as a compression
    ratio). \textcolor{red}{Vasya, please, instead of s as x-axis please make it
    the the resize retio (i.e., from 0 to 1 where 1 is the originial largest
    image), also please inset the original image you used, remove (s) on the
    y-axis.}}
  \label{fig:crit-plot}
\end{figure}
...
\begin{figure}[ht]
  \centering \includegraphics[width=0.99\linewidth]{images/kernels.png}
